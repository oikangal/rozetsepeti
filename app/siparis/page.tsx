...
const OK_FILE_TYPES = [
    ...
    'image/png',
    'image/tiff',
    // bazı tarayıcılar .ai için tip göndermeyebilir → uzantı kontrolüyle de kabul edeceğiz
] as const
...
export default function SiparisPage() {
    ...
    const onSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        // --- Dosya kontrolleri (tip + 20MB limit) ---
        ...
        for (const f of files) {
            ...
            const isAi = name.endsWith('.ai') // Illustrator dosyası
            const typeOk = OK_FILE_TYPES.includes(f.type as any) || isAi
            if (!typeOk) {
        ...
        // --- Min adet kontrolü (iş kuralı: min 10) ---
        const adetRaw = String(fd.get('adet') ?? '')
        const adet = parseInt(adetRaw, 10)
        if (!Number.isFinite(adet) || adet < 10) {
            setState({ status: 'error', message: 'Minimum adet 10 olmalıdır.' })
            return
        }
            ...
            <form onSubmit={onSubmit} className="mt-6 grid gap-4" noValidate>
                ...
                <fieldset className="grid gap-2">
                    ...
                    <div className="grid sm:grid-cols-2 gap-2">
                        {URUNLER.map((u) => (
                            <label
                                key={u.key}
                                className="inline-flex items-center gap-2 rounded-2xl border px-3 py-2 cursor-pointer hover:border-[var(--brand)]"
                            >
                ...
                <label className="grid gap-1 text-sm">
                    ...
                    <input
                        ...
                        inputMode="numeric"
                    />
                    <span className="text-xs text-gray-500">Minimum 10 adettir.</span>
                </label>
              ...
              text-sm font-medium shadow-sm text-white disabled:opacity-60
            "
